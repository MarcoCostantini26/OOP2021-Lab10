...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        List<Optional<T>> listFilter = new ArrayList<>();
        list.forEach(value -> listFilter.add(Optional.ofNullable(value).filter(pre)));
        /*
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        Map<R, Set<T>> mapGroup = new HashMap<>();
        list.forEach(value -> mapGroup.merge(op.apply(value), new HashSet<T>(Arrays.asList(value)),
                (x1, x2) -> {
                    x1.addAll(x2);
                    return x1;
                }));
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((k, v) -> {
            fillMap.put(k, v.orElse(def.get()));
        });
        /*
